...
impl LiquidityPool {
    ...
    pub fn remove_liquidity(e: Env, caller: Address, liquidity: i128) -> (i128, i128) {
        ...
        if pool_info.is_xlm_pool {
            ...
            if amount_a > 0 {
                let caller_muxed = MuxedAddress::from(caller.clone());
                token_a_client.transfer(&e.current_contract_address(), &caller_muxed, &amount_a);
            }
            if amount_b > 0 {
                let caller_muxed = MuxedAddress::from(caller.clone());
                token_b_client.transfer(&e.current_contract_address(), &caller_muxed, &amount_b);
            }
        }
    ...
    pub fn swap(e: Env, caller: Address, input_token: Address, amount_in: i128) -> i128 {
        ...
        if pool_info.is_xlm_pool && (is_native_xlm(&token_in) || is_native_xlm(&token_out)) {
            ...
            let caller_muxed = MuxedAddress::from(caller.clone());
            token_out_client.transfer(&e.current_contract_address(), &caller_muxed, &amount_out);
        }
    ...
    pub fn claim_fees(e: Env, caller: Address) -> i128 {
        ...
        if pool_info.is_xlm_pool {
            ...
            if user_fee_share_a > 0 {
                let caller_muxed = MuxedAddress::from(caller.clone());
                token_a_client.transfer(&e.current_contract_address(), &caller_muxed, &user_fee_share_a);
            }
            if user_fee_share_b > 0 {
                let caller_muxed = MuxedAddress::from(caller.clone());
                token_b_client.transfer(&e.current_contract_address(), &caller_muxed, &user_fee_share_b);
            }
        }
    ...
    fn handle_xlm_fee_transfer(e: &Env, caller: &Address, pool_info: &PoolInfo, fee_share_a: i128, fee_share_b: i128) {
        match pool_info.xlm_token_index {
            Some(0) => {
                ...
                if fee_share_b > 0 {
                    let caller_muxed = MuxedAddress::from(caller.clone());
                    token_b_client.transfer(&e.current_contract_address(), &caller_muxed, &fee_share_b);
                }
            ...
            Some(1) => {
                ...
                if fee_share_a > 0 {
                    let caller_muxed = MuxedAddress::from(caller.clone());
                    token_a_client.transfer(&e.current_contract_address(), &caller_muxed, &fee_share_a);
                }
                // Transfer XLM fees (only if amount > 0)
                if fee_share_b > 0 {
...
impl token::Interface for LiquidityPool {
    ...
    fn transfer(e: Env, from: Address, to: MuxedAddress, amount: i128) {
        ...
        let to_balance = Self::balance(e.clone(), to_addr.clone());
        let new_to_balance = checked_add(to_balance, amount);
        
        e.storage().instance().set(&DataKey::Balance(from), &new_from_balance);
        e.storage().instance().set(&DataKey::Balance(to_addr), &new_to_balance);
    }
